<x-app-layout>
    ...
    <div class="contenedor-principal">
        ...
        <div class="container mt-4" style="height: 20%">
            <h2 class="titulo_formulario">Cargue Masivo de Llamados de Atención</h2>
    
            @if(session('success'))
                <div class="alert alert-success">{{ session('success') }}</div>
            @endif
    
            <form action="{{ route('llamados.importar') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="archivo" class="form-label">Archivo Excel:</label>
                    <input type="file" name="archivo" class="form-control" required>
                </div>
                <button type="submit" class="btn btn-success">📥 Importar Llamados</button>
            </form>
        </div>
