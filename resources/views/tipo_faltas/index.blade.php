<x-app-layout>
    ...
    <div class="contenedor-principal">
        ...
        <h2 class="mb-4">Listado de Tipos de Falta</h2>

        <a href="{{ route('tipo_faltas.create') }}" class="btn btn-success mb-3">
            ...
            <tbody>
                @foreach($tipos as $tipo)
                    <tr>
                        <td>{{ $tipo->nombre }}</td>
                        <td>
                            <a href="{{ route('tipo_faltas.edit', $tipo->id) }}" class="btn btn-sm btn-primary">Editar</a>
                            <form action="{{ route('tipo_faltas.destroy', $tipo->id) }}" method="POST" style="display:inline;">
                                @csrf @method('DELETE')
